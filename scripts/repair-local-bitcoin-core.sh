...
usage() {
  cat <<'EOF'
Usage: scripts/repair-local-bitcoin-core.sh [option]
...
Options:
  ...
  --yes-reset-local-bitcoin-data
      Acknowledge local Bitcoin Core data loss for --reset-volume.
...
case "$MODE" in
  reindex-chainstate)
    ...
    info "Follow progress with: docker logs -f bitcoin-pruned-node-local"
    ;;
  ...
  reset-volume)
    ...
    info "When Bitcoin Core is running, start the full stack with: bash scripts/start-local.sh --no-build"
    ;;

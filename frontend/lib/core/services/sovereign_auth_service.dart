...
class SovereignAuthService {
  ...
  static const String errorNoLocalCredentials =
      'ERR_AUTH_PASSKEY_NO_LOCAL_CREDENTIALS';
  static const String errorAuthCancelled = 'ERR_AUTH_PASSKEY_AUTH_CANCELLED';
  ...
  Future<Uint8List> generateKeyPair() async {
    try {
      ...
      await _storage.write(key: _signatureCounterStorageKey, value: '0');

      debugPrint(
          '🔐 [SovereignAuth] New Ed25519 key pair generated and stored.');
  ...
  Future<Uint8List> signBytes(Uint8List data) async {
    try {
      ...
      final didAuthenticate = await _localAuth.authenticate(
        localizedReason:
            'Autentique-se para assinar com sua chave de segurança.',
        biometricOnly: false,
      ...
      debugPrint(
        '❌ [SovereignAuth] Local auth error signing data: ${e.code.name}'
        '${e.description == null ? '' : ' (${e.description})'}',
      );
      throw Exception(_mapLocalAuthException(e));
    } catch (e) {
  ...
  Future<int> nextSignatureCounter() async {
    final currentRaw = await _storage.read(key: _signatureCounterStorageKey);
  ...
  String _mapLocalAuthException(LocalAuthException error) {
    switch (error.code) {
      case LocalAuthExceptionCode.noCredentialsSet:
      case LocalAuthExceptionCode.noBiometricHardware:
      case LocalAuthExceptionCode.noBiometricsEnrolled:
        return errorNoLocalCredentials;
      case LocalAuthExceptionCode.userCanceled:
      case LocalAuthExceptionCode.systemCanceled:
      case LocalAuthExceptionCode.userRequestedFallback:
        return errorAuthCancelled;
      default:

...
class _ReceiveScreenState extends ConsumerState<ReceiveScreen> {
  ...
  String _flowEyebrow(BuildContext context) {
    switch (widget.initialMode) {
      case ReceiveFlowMode.qrCode:
        return context.tr.receiveScreenQrEyebrow;
      case ReceiveFlowMode.nfc:
        return context.tr.nfc.toUpperCase();
      case ReceiveFlowMode.paymentLink:
        return context.tr.receiveScreenPaymentLinkEyebrow;
      case ReceiveFlowMode.onChain:
        return context.tr.receiveScreenOnchainEyebrow;
      case ReceiveFlowMode.lightning:
        return context.tr.receiveScreenLightningEyebrow;
    }
  }
  ...
  String _flowDescription(BuildContext context) {
    switch (widget.initialMode) {
      case ReceiveFlowMode.qrCode:
        return context.tr.receiveScreenQrDescription;
      case ReceiveFlowMode.nfc:
        return context.tr.receiveScreenNfcDescription;
      case ReceiveFlowMode.paymentLink:
        return context.tr.receiveScreenPaymentLinkDescription;
      case ReceiveFlowMode.onChain:
        return context.tr.receiveScreenOnchainDescription;
      case ReceiveFlowMode.lightning:
        return context.tr.receiveScreenLightningDescription;
    }
  }
  ...
  String _continueLabel(BuildContext context) {
    switch (widget.initialMode) {
      case ReceiveFlowMode.qrCode:
        return context.tr.receiveScreenGenerateQr;
      case ReceiveFlowMode.nfc:
        return context.tr.receiveScreenPrepareNfc;
      case ReceiveFlowMode.paymentLink:
        return context.tr.receiveScreenCreateLink;
      case ReceiveFlowMode.onChain:
        return context.tr.receiveScreenGenerateOnchainQr;
      case ReceiveFlowMode.lightning:
        return context.tr.receiveScreenGenerateLightningInvoice;
    }
  }
  ...
  void _openDeposit() {
    ...
    if (wallet == null) {
      SnackbarHelper.showError(context.tr.receiveScreenSelectDepositWallet);
      return;
    }
  ...
  String _screenSubtitle(BuildContext context) {
    switch (widget.initialMode) {
      case ReceiveFlowMode.qrCode:
        return context.tr.receiveScreenQrSubtitle;
      case ReceiveFlowMode.nfc:
        return context.tr.receiveScreenNfcSubtitle;
      case ReceiveFlowMode.paymentLink:
        return context.tr.receiveScreenPaymentLinkSubtitle;
      case ReceiveFlowMode.onChain:
        return context.tr.receiveScreenOnchainSubtitle;
      case ReceiveFlowMode.lightning:
        return context.tr.receiveScreenLightningSubtitle;
    }
  }
  ...
  Widget _buildInboundBlockedCard(dynamic activationStatus) {
    ...
    final warning = rawWarning != null && rawWarning.isNotEmpty
        ? ErrorTranslator.translate(context.tr, rawWarning)
        : context.tr.receiveScreenInboundBlockedMessage;
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      context.tr.receiveScreenInboundBlockedTitle,
          ...
          Wrap(
            ...
            children: [
              ...
              TextButton.icon(
                ...
                label: Text(context.tr.receiveScreenRefreshStatus),
                style: TextButton.styleFrom(
                  foregroundColor: receiveFlowMutedTextColor,
                ),
              ),
    ...
    return ReceiveFlowPanel(
      child: Column(
        children: [
          ...
          if (_selectedCurrency != Currency.btc)
            Text(
              context.tr.receiveScreenEquivalentTo(
          ...
          if (wallet != null && btcEquivalent > 0) ...[
            const SizedBox(height: AppSpacing.md),
            ReceiveFlowTag(
              label: context.tr.receiveScreenDestination(wallet.name),
              icon: LucideIcons.lock,
            ),
            const SizedBox(height: 8),
            Text(
              context.tr.receiveScreenPrivacyHint,
              style: Theme.of(context).textTheme.bodySmall?.copyWith(
  ...
  Future<_PaymentLinkConfig?> _showPaymentLinkConfigSheet() {
    final descriptionController = TextEditingController(
      text: context.tr.receiveScreenDefaultDescription(
                ...
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        context.tr.receiveScreenConfigureLinkEyebrow,
                      ...
                      const SizedBox(height: AppSpacing.sm),
                      Text(
                        context.tr.receiveScreenConfigureLinkSubtitle,
                        style: Theme.of(context).textTheme.bodySmall?.copyWith(
                              color: monoMutedTextColor,
                              height: 1.45,
                            ),
                      ...
                      TextField(
                        ...
                        decoration: monochromeInputDecoration(
                          label: context.tr.receiveScreenDescriptionLabel,
                        ),
                      ),
                      TextField(
                        ...
                        style: const TextStyle(color: monoTextColor),
                        decoration: monochromeInputDecoration(
                          label: context.tr.receiveScreenReferenceLabel,
                        ),
                      ),
                      const SizedBox(height: AppSpacing.md),
                      DropdownButtonFormField<int>(
                        ...
                        items: [
                          DropdownMenuItem(
                            value: 15,
                            child: Text(context.tr.receiveScreen15Minutes),
                          ),
                          DropdownMenuItem(
                            value: 60,
                            child: Text(context.tr.receiveScreen1Hour),
                          ),
                          DropdownMenuItem(
                            value: 180,
                            child: Text(context.tr.receiveScreen3Hours),
                          ),
                          DropdownMenuItem(
                            value: 1440,
                            child: Text(context.tr.receiveScreen24Hours),
                          ),
                        ],
                        onChanged: (value) {
                          if (value != null) {
                        ...
                        decoration: monochromeInputDecoration(
                          label: context.tr.receiveScreenValidityLabel,
                        ),
                      ),
                      const SizedBox(height: AppSpacing.md),
                      DropdownButtonFormField<String>(
                        ...
                        items: [
                          DropdownMenuItem(
                            value: 'PRIVATE',
                            child: Text(context.tr.receiveScreenPrivate),
                          ),
                          DropdownMenuItem(
                            value: 'PUBLIC',
                            child: Text(context.tr.receiveScreenPublic),
                          ),
                        ],
                        onChanged: (value) {
                          if (value != null) {
                        ...
                        decoration: monochromeInputDecoration(
                          label: context.tr.receiveScreenVisibilityLabel,
                        ),
                      ),
                      const SizedBox(height: AppSpacing.md),
                      DropdownButtonFormField<String>(
                        ...
                        items: [
                          DropdownMenuItem(
                            ...
                            child: Text(
                              context.tr.receiveScreenUserActionRequired,
                            ),
                          ),
                          DropdownMenuItem(
                            value: 'AUTO_COMPLETE',
                            child: Text(
                              context.tr.receiveScreenAutoComplete,
                            ),
                          ),
                        ],
                        onChanged: (value) {
                        ...
                        decoration: monochromeInputDecoration(
                          label: context.tr.receiveScreenCompletionLabel,
                        ),
                      ),
                      const SizedBox(height: AppSpacing.md),
                      TextField(
                        ...
                        style: const TextStyle(color: monoTextColor),
                        decoration: monochromeInputDecoration(
                          label: context.tr.receiveScreenCustomerLabel,
                        ),
                      ),
                      TextField(
                        ...
                        style: const TextStyle(color: monoTextColor),
                        decoration: monochromeInputDecoration(
                          label: context.tr.receiveScreenNoteLabel,
                        ),
                      ),
                      const SizedBox(height: AppSpacing.lg),
                      FilledButton(
                        ...
                        child: Text(context.tr.receiveScreenGenerateLink),
                      ),
                    ],
                  ),
                ),

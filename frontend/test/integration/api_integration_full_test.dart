  ...
  group('API Integration & Contract Tests (Real Onion Data)', () {
    ...
    setUpAll(() async {
      ...
      try {
        ...
        if (!torStarted) {
          print('⚠️ [Setup] Tor failed to start. Tests requiring .onion will fail.');
          return;
        }
    ...
    test('Contract: GET /auth/pow/challenge returns valid PoW schema', () async {
      final apiClient = container.read(apiClientProvider);
      
      try {
        print('📡 [Test] Requesting PoW challenge from ${AppConfig.apiUrl}${AppConfig.authPowChallenge}');
        ...
        expect(data.containsKey('challenge'), true, reason: 'Missing challenge field');
        expect(data.containsKey('difficulty'), true, reason: 'Missing difficulty field');
    ...
    test('WebSocket: Connectivity check to Price Feed via Relay', () async {
      ...
      print('🔌 [Test] Attempting TCP Connection to Relay Port $relayPort (WS Tunnel)');
      
      try {
        final socket = await Socket.connect('127.0.0.1', relayPort, timeout: const Duration(seconds: 10));

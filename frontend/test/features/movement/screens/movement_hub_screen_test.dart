...
void main() {
  testWidgets('lists up to three receive wallets from the wallet provider',
      (tester) async {
    await _pumpMovementHubScreen(tester, initialWallet: _internalWallet());
  ...
  testWidgets('filters receive methods by selected wallet type',
    ...
    await _pumpMovementHubScreen(tester, initialWallet: _coldWallet());
    await tester.pumpAndSettle();
...
Future<void> _pumpMovementHubScreen(
  WidgetTester tester, {
  ...
  return tester.pumpWidget(
    ProviderScope(
      ...
      child: MaterialApp(
        ...
        home: MovementHubScreen(initialWallet: initialWallet),
      ),
    ),
  );
}

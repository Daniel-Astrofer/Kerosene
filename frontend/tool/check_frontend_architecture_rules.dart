...
void _checkForbiddenFeatureArchiveDirs(List<_Violation> violations) {
  for (final path in const [
    'lib/features/_legacy',
    'lib/features/_roadmap',
    'lib/features/transactions',
    'lib/features/wallet',
    'lib/features/bitcoin_accounts',
    'lib/core/widgets',
  ]) {
    if (Directory(path).existsSync()) {
      violations.add(_Violation(
        'forbidden_legacy_frontend_dir',
        path,
        'This frontend area was migrated to the KFE-only architecture and must not be reintroduced.',
      ));
    }
  }
}
